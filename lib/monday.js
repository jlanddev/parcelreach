...
const ALLOWED_BOARDS = ['ls', 'land exit solutions', 'plg', 'wwlc'];
...
const BOARD_MENTIONS = {
  ...
  '6560811435': [{ userId: 60629994, name: 'Chris Newton' }, { userId: 60630018, name: 'Spencer Anderson' }], // PLG (Pyramid Properties) -> Chris + Spencer
  '6560808945': [{ userId: 60765276, name: 'Christopher Gibson' }], // WWLC (Wild West Land Company) -> Gibson
};
...
// Is an item still LIVE on the given board? Partners delete a pushed lead in
// their CRM, but Monday only soft-deletes: the item goes to the recycle bin and
// the API still resolves it by id. So "exists" is not enough. We must confirm
// the item's state is `active` (not deleted/archived) and that it still lives on
// the target board; otherwise it is a ghost and we recreate rather than post an
// update into their trash.
export async function itemIsLive(itemId, boardId) {
  if (!itemId) return false;
  try {
    const data = await mondayQuery('query ($id:[ID!]) { items(ids:$id) { id state board { id } } }', { id: [String(itemId)] });
    const it = data.items && data.items[0];
    if (!it || !it.id) return false;
    if (it.state && it.state !== 'active') return false;            // deleted or archived -> ghost
    if (boardId && it.board?.id && String(it.board.id) !== String(boardId)) return false; // moved off board
    return true;
  } catch { return false; }
}
...
// The exact group a lead should land in, per board (matched by title,
// case-insensitive). Explicit so a lead never falls into a reference group.
const BOARD_GROUPS = {
  '6560811435': /working deals/i, // PLG -> Working Deals
};

// Pick the group new leads should land in: the per-board target first, then an
// obvious leads/deals group, then any non-reference/archive group (so we never
// dump a lead into a "Resources" bucket), then the first group as a last resort.
function pickGroup(groups = [], boardId) {
  const pref = BOARD_GROUPS[String(boardId)];
  if (pref) {
    const hit = groups.find((g) => pref.test(g.title));
    if (hit) return hit;
  }
  const bad = /resource|archive|done|complete|template|reference|closed|dead|lost|link|doc|industry|contact|funding|document/i;
  return (
    groups.find((g) => /new lead|lead in|inbound|review/i.test(g.title)) ||
    groups.find((g) => /working deal|deal|offer|active|project|pipeline|prospect/i.test(g.title) && !bad.test(g.title)) ||
    groups.find((g) => !bad.test(g.title)) ||
    groups[0]
  );
}
...
// Push a lead to a partner board. Paths:
//  - FIRST push (no existingItemId): create the item + columns + update bubble
//    (the @mention tag + summary) + parcel map image + notification.
//  - FOLLOW-UP (existingItemId given): the partner already has this lead, so we
//    post the new note as an update to that item's thread and ping them. No
//    duplicate item, no columns. The map IS re-sent if the first push never
//    included one (opts.priorMapUploaded === false), e.g. it was pushed before
//    the map was attached.
//  - RECREATE: if the stored item was deleted on the partner's board (or
//    opts.forceNew is set), we fall back to a full first push and the caller
//    repoints item_id to the new item.
// Returns a per-step report so the UI can show exactly what landed.
export async function pushLeadToBoard(boardId, lead, opts = {}) {
  const forceNew = !!opts.forceNew;
  const priorMapUploaded = !!opts.priorMapUploaded;
  let existingItemId = (!forceNew && opts.existingItemId) ? String(opts.existingItemId) : null;

  const meta = await getBoardMeta(boardId);
  if (!meta) throw new Error('Board not found');

  // If we think this lead already lives on the board, make sure it is actually
  // LIVE there (not sitting in the partner's recycle bin or moved off the board).
  // A ghost means we recreate instead of updating into their trash.
  let recreated = false;
  if (existingItemId && !(await itemIsLive(existingItemId, boardId))) {
    existingItemId = null;
    recreated = true;
  }
  ...
  // The partner summary (composed on the card in the Partners tab) is the ONLY
  // note text we push, so internal notes never leak.
  const summary = (lead.partner_summary || '').trim();
  const summaryHtml = summary ? escapeHtml(summary).replace(/\n/g, '<br>') : '';
  const coords = (lead.partner_coordinates || '').trim();
  const coordsHtml = coords ? `Coordinates: ${escapeHtml(coords)}` : '';

  // Resolve the item: reuse the existing one on a follow-up, else create it.
  let itemId;
  let group = null;
  if (isUpdate) {
    itemId = existingItemId;
  } else {
    group = pickGroup(meta.groups, boardId);
    if (!group) throw new Error('No group on board');
    const columnValues = buildColumnValues(meta.columns, lead);
    itemId = await createItem(boardId, group.id, itemName, columnValues);
  }

  // Default line differs so a follow-up does not read as a brand new lead.
  const defaultLine = isUpdate ? 'Update from ParcelReach' : 'New lead from ParcelReach';

  // 1) The update bubble carrying the @mention tag (+ summary). If Monday
  // rejects the mention HTML for any reason, retry with a plain-text body.
  let updateId = null;
  let updatePosted = false;
  let tagInBubble = false;
  let inner = tags;
  if (summaryHtml) inner += (inner ? '<br>' : '') + summaryHtml;
  if (coordsHtml) inner += (inner ? '<br>' : '') + coordsHtml;
  const tagBody = `<p>${inner || defaultLine}</p>`;
  try {
    ...
    try {
      const lead_line = tagged ? `${defaultLine} (for ${tagged})` : defaultLine;
      let plain = lead_line;
      if (summary) plain += `\n${summary}`;
      if (coords) plain += `\nCoordinates: ${coords}`;
  ...
  // 2) The parcel map image. Sent on a first push/recreate, and ALSO on a
  // follow-up when the first push never carried a map (priorMapUploaded false),
  // which is the "pushed before I attached the map" case. Skipped on a
  // follow-up only when the map already went over.
  const sendMap = !!lead.map_image_url && !!updateId && (!isUpdate || !priorMapUploaded);
  ...
  } else if (!lead.map_image_url && !isUpdate) {
    warnings.push('no map on this lead to push');
  }

  // 2b) Extra attachments the rep added (a new map, a survey, any doc). These go
  // on BOTH a first push and a follow-up, so a partner can be sent an updated
  // map or a document at any point in the deal.
  let filesUploaded = 0;
  const attachments = Array.isArray(opts.attachments) ? opts.attachments : [];
  if (updateId && attachments.length) {
    for (const a of attachments) {
      if (!a?.url) continue;
      try { await addFileToUpdate(updateId, a.url, a.filename || 'attachment'); filesUploaded++; }
      catch (e) { warnings.push(`attachment "${a.filename || a.url}" failed: ` + (e?.message || e)); }
    }
  }
  ...
  let notified = 0;
  const notifyText = isUpdate ? `Update from ParcelReach: ${itemName}` : `New lead from ParcelReach: ${itemName}`;
  for (const m of mentions) {
    try { await createNotification(m.userId, itemId, notifyText); notified++; }
  ...
  try {
    ...
    else if (sendMap && !ups.some((u) => (u.assets || []).length)) warnings.push('verify: update has no file attached');
  } catch (e) {
    warnings.push('verify failed: ' + (e?.message || e));
  }

  return { itemId, board: meta.name, group: group?.title || null, updatedExisting: isUpdate, recreated, updatePosted, tagInBubble, mapUploaded, filesUploaded, tagged, notified, verifiedUpdates, warnings };
}
