...
// The bubble's note text = our genuine collaborative notes only. System activity
// logs ([VM], [CALL], [TEXT] …) are filtered out. Returns '' when there are none,
// so the bubble is then just the tag and the map.
function buildUpdateBody(lead) {
  const notes = Array.isArray(lead.notes) ? lead.notes : [];
  return notes
    .map((n) => (n.content || '').trim())
    .filter((t) => t && !/^\[[A-Za-z]+\]/.test(t)) // drop [VM]/[CALL]/[TEXT] logs
    .join('\n');
}
...
export async function pushLeadToBoard(boardId, lead) {
  ...
  const noteText = buildUpdateBody(lead);
  const mentions = BOARD_MENTIONS[String(boardId)] || [];
  const tags = mentions.map(mentionTag).join(' ');
  const notesHtml = noteText ? escapeHtml(noteText).replace(/\n/g, '<br>') : '';
  let inner = tags;
  if (notesHtml) inner += (tags ? '<br>' : '') + notesHtml;
  const body = `<p>${inner || 'Lead from ParcelReach'}</p>`;

  let mapUploaded = false;
  let tagged = mentions.map((m) => m.name).join(', ') || null;
  ...
  // Explicitly notify each tagged partner so they actually get pinged (API
  // update mentions alone don't fire a notification).
  for (const m of mentions) {
    await createNotification(m.userId, itemId, `New lead from ParcelReach: ${itemName}`);
  }
