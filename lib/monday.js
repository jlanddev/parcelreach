...
// notifies the partner. TESTING: both point at Jordan so weekend tests don't
// ping a partner. For real, switch LS -> D. Browne (60588779), Land Exit
// Solutions -> Larry Jarnigo (60686744).
const BOARD_MENTIONS = {
  '6560911783': { userId: 58708421, name: 'Jordan Harmon' },   // LS (real = D. Browne 60588779)
  '6560915772': { userId: 58708421, name: 'Jordan Harmon' },   // Land Exit Solutions (real = Larry Jarnigo 60686744)
};
...
// The bubble text: the lead's note summary (our collaborative notes) plus the
// why-selling motivation. Property fields already live in the columns, so we
// don't repeat them here.
function buildUpdateBody(lead) {
  const why = lead.form_data?.whySelling || lead.why_selling || '';
  const notes = Array.isArray(lead.notes) ? lead.notes : [];
  const lines = [];
  if (why) lines.push(`Why selling: ${why}`);
  if (notes.length) {
