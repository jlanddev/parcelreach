...
export async function pushLeadToBoard(boardId, lead) {
  ...
  const group = pickGroup(meta.groups, boardId);
  ...
  // The partner summary (composed and locked on the card in the Partners tab)
  // is the ONLY note text we push, so internal notes never leak. Format:
  // @Partner, then the summary, then coordinates (if any), then the map below.
  ...
  // rejects the mention HTML for any reason, retry with a plain-text body so the
  // bubble (and the map) still land. We then rely on create_notification.
  let updateId = null;
  ...
  const tagBody = `<p>${inner || 'New lead from ParcelReach'}</p>`;
  try {
    ...
    try {
      const lead_line = tagged ? `New lead from ParcelReach (for ${tagged})` : 'New lead from ParcelReach';
      let plain = lead_line;
      if (summary) plain += `\n${summary}`;
      if (coords) plain += `\nCoordinates: ${coords}`;
