...
// Push a lead to a partner board. Two paths:
...
//    only post the new note as an update to that item's thread and ping them.
//    No duplicate item, no columns, and NO map (the map went on the first push).
// Returns a per-step report so the UI can show exactly what landed.
export async function pushLeadToBoard(boardId, lead, opts = {}) {
  const existingItemId = opts.existingItemId ? String(opts.existingItemId) : null;
  ...
  // 2) The parcel map image. ONLY on the first push; follow-up updates never
  // re-send the map.
  ...
  try {
    ...
    else if (!isUpdate && lead.map_image_url && !ups.some((u) => (u.assets || []).length)) warnings.push('verify: update has no file attached');
  } catch (e) {
    warnings.push('verify failed: ' + (e?.message || e));
  }

  return { itemId, board: meta.name, group: group?.title || null, updatedExisting: isUpdate, updatePosted, tagInBubble, mapUploaded, tagged, notified, verifiedUpdates, warnings };
}
