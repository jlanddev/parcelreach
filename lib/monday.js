...
// Does an item still exist on the board? Partners sometimes delete a pushed
// lead in their CRM; when they do, our stored item_id is a ghost and we must
// recreate rather than post an update into the void.
export async function itemExists(itemId) {
  if (!itemId) return false;
  try {
    const data = await mondayQuery('query ($id:[ID!]) { items(ids:$id) { id } }', { id: [String(itemId)] });
    return !!(data.items && data.items.length && data.items[0]?.id);
  } catch { return false; }
}
...
export async function pushLeadToBoard(boardId, lead, opts = {}) {
  ...
  // If we think this lead already lives on the board, make sure it really does.
  // A deleted item means we recreate instead of updating a ghost.
  let recreated = false;
  if (existingItemId && !(await itemExists(existingItemId))) {
    existingItemId = null;
    recreated = true;
  }
