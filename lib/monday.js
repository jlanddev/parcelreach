...
export async function pushLeadToBoard(boardId, lead) {
  ...
  // Compose the update bubble: ONLY the @mention tag and the map image.
  // We deliberately do NOT push our internal notes to the partner board.
  const mentions = BOARD_MENTIONS[String(boardId)] || [];
  const tags = mentions.map(mentionTag).join(' ');
  const body = `<p>${tags || 'New lead from ParcelReach'}</p>`;
