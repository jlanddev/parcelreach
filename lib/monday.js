...
const ALLOWED_BOARDS = ['ls', 'land exit solutions', 'plg'];
...
// Pick the group new leads should land in. Prefer an obvious leads/deals group,
// then any group that isn't a reference/archive bucket (so we never dump a lead
// into a "Resources" group like PLG's), then the first group as a last resort.
function pickGroup(groups = []) {
  const bad = /resource|archive|done|complete|template|reference|closed|dead|lost|link|doc/i;
  return (
    groups.find((g) => /new lead|lead in|inbound|review/i.test(g.title)) ||
    groups.find((g) => /deal|offer|active|project|pipeline|prospect/i.test(g.title) && !bad.test(g.title)) ||
