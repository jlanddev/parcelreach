...
export async function findDeals(params = {}, { noCache = false } = {}) {
  ...
  // Wide price-per-acre ladder so metro markets (small lots >$100k/ac) get a real
  // median instead of capping at the top bracket. Empty brackets cost 0 quota.
  const EDGES = [5000, 10000, 20000, 40000, 75000, 125000, 200000, 350000, 600000];
  ...
  const perCounty = await Promise.all(fips.map(async (county) => {
    ...
    if (ceiling) {
      const parents = await rawFilter({ fips: [county], filters: [
        ...
        ...(isNum(frontageMin) ? [range('road_frontage', { min: frontageMin })] : []),
      ] });
