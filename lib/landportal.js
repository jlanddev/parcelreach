...
export async function findDeals(params = {}, { noCache = false } = {}) {
  ...
  const EDGES = [2500, 5000, 7500, 10000, 15000, 20000, 30000, 50000, 80000];
  ...
  for (const county of fips) {
    ...
    const cumCounts = cum.map((r) => r.count || 0);
    const medianPPA = estimateMedian(EDGES, cumCounts, N);
    ...
    counties.push({ fips: county, soldCount: N, medianPPA, ceiling, listingCount });
    for (const p of listings) {
      const id = String(p.property_id ?? `${p.fips}:${p.apn}`);
      if (!seen.has(id)) { seen.add(id); allProps.push({ ...p, __ceiling: ceiling, __medianPPA: medianPPA }); }
    }
  }
