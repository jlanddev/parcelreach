...
async function getQuotaToday() {
  ...
  try {
    ...
    if (data) return { pool_remaining: data.pool_remaining, tokens_spent_cents: data.tokens_spent_cents || 0, fetches: data.fetches || 0 };
...
export async function runHydration(items = [], { cap = HYDRATION.RUN_CAP_DEFAULT } = {}) {
  ...
  let fetched = 0, spentCents = 0, stoppedShort = false;
  for (const it of newItems) {
    const before = poolRemainingEst(q);
    // GUARD 3: re-check the breaker each iteration in case of drift.
    if (q.tokens_spent_cents >= HYDRATION.DAILY_TOKEN_CEILING_CENTS) { stoppedShort = true; break; }
    let res;
    try {
      res = await getPropertyDetail(it.property_id, it.fips);
    } catch (e) {
      if (e instanceof LandPortalError && e.code === 'forbidden') { stoppedShort = true; break; } // pool+tokens exhausted
      throw e;
    }
