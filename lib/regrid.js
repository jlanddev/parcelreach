...
export async function fetchParcelGeometry({ apn, fips, state, county, address, lat, lon } = {}) {
  const t = token();
  if (!t) return null;
  const abbr = resolveAbbr({ state, fips });
  const path = (abbr && county) ? `/us/${abbr}/${slug(county)}` : null;
  ...
  const cacheKey = apn ? `apn:${String(fips || '').slice(0, 2)}:${slug(county)}:${String(apn).trim()}`
    : (lat && lon ? `pt:${lat}:${lon}` : (address && path ? `adr:${path}:${String(address).trim().toLowerCase()}` : null));
  if (cacheKey) {
    try {
  ...
  // Fallback chain: APN (fastest) -> street address (handles counties whose APN
  // numbering differs from Regrid, e.g. Wake NC PINs) -> lat/lon point.
  let geometry = null;
  if (apn && path) {
    geometry = await tryUrl(`${REGRID}/parcels/apn?parcelnumb=${enc(String(apn).trim())}&path=${enc(path)}&return_geometry=true&limit=1&token=${t}`);
  }
  if (!geometry && address && path && /\d/.test(String(address))) {
    geometry = await tryUrl(`${REGRID}/parcels/address?query=${enc(String(address).trim())}&path=${enc(path)}&return_geometry=true&limit=1&token=${t}`);
  }
  if (!geometry && lat && lon) {
    geometry = await tryUrl(`${REGRID}/parcels/point?lat=${enc(lat)}&lon=${enc(lon)}&return_geometry=true&limit=1&token=${t}`);
  }
