...
export async function findLeadByPhone(supabase, phone) {
  ...
  const { data } = await supabase
    .from('leads')
    .select('id, phone, owner_phone, status, pipeline_status, owner_name, full_name, name, current_owner_id')
    .or(`phone.ilike.*${last4},owner_phone.ilike.*${last4}`)
    .limit(50);
  if (!data) return null;
  const norm = (p) => (p || '').replace(/\D/g, '').slice(-10);
  return data.find((l) => norm(l.phone) === d || norm(l.owner_phone) === d) || null;
}
