...
/** Most recent messages across ALL contacts (one call) — used to compute each
 * lead card's last-contacted summary + unread without N per-lead requests. */
export async function getRecentMessages({ limit = 100 } = {}) {
  const res = await pb('/get-messages-api', {
    query: { limit: Math.min(limit, 100), order_by: 'createdAt', order_direction: 'desc' },
  }).catch(() => ({ data: [] }));
  return res.data || [];
}
