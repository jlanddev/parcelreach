...
export function buildFilterRequest(p = {}) {
  ...
  if (p.status === 'for_sale') {
    const days = isNum(p.listed_within_days) ? Number(p.listed_within_days) : 30;
    filters.push({ key: 'mls_for_sale', operator: 'mls_condition', value: 'active', days });
    submittedMlsKeys.push('mls_for_sale');
  } else if (p.status === 'sold') {
    const days = isNum(p.sold_within_days) ? Number(p.sold_within_days) : 365;
    filters.push({ key: 'mls_sold', operator: 'mls_condition', value: 'sold', days });
  ...
  if (isMarketQuery || mlsTypes) {
    filters.push(condition('mls_propertytype', mlsTypes || [LAND_PROPERTY_TYPE]));
